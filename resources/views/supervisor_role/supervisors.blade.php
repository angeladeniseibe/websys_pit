...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .branch-banner { display: flex; align-items: flex-start; gap: 9px; background: #E6F1FB; border-radius: 8px; padding: 10px 14px; margin-bottom: 14px; font-size: 11px; color: #0C447C; line-height: 1.6; }
        .branch-banner i { font-size: 15px; flex-shrink: 0; margin-top: 1px; color: #185FA5; }
        .readonly-note { display: flex; align-items: flex-start; gap: 9px; background: #FEF8F7; border: 0.5px solid #E8D0CE; border-radius: 8px; padding: 10px 14px; margin-bottom: 14px; font-size: 11px; color: #993556; line-height: 1.6; }
        .stat-row { background: #fff; border: 0.5px solid #E8D0CE; border-radius: 10px; padding: 12px 14px; display: flex; align-items: center; gap: 10px; margin-bottom: 14px; }
        ...
        .search-input { width: 100%; background: #fff; border: 0.5px solid #C4A8A4; border-radius: 8px; padding: 8px 12px 8px 34px; font-size: 12px; color: #3d2030; font-family: 'DM Sans', sans-serif; outline: none; }
        ...
        .card-head { display: flex; align-items: center; justify-content: space-between; padding: 11px 18px; border-bottom: 0.5px solid #EDD6D4; background: #FEF8F7; }
        ...
        .badge-highlight { background: #FAEEDA; color: #633806; }
        .tbl-wrap { overflow-x: auto; }
        table { width: 100%; border-collapse: collapse; }
        th { padding: 9px 16px; text-align: left; font-size: 9px; font-weight: 600; color: #993556; text-transform: uppercase; letter-spacing: 0.07em; border-bottom: 0.5px solid #EDD6D4; background: #FEF8F7; white-space: nowrap; }
        ...
        tr.is-me td { background: #FBEAF0; }
        .av-wrap { display: flex; align-items: center; gap: 8px; }
        .av { width: 26px; height: 26px; border-radius: 50%; background: linear-gradient(135deg, #F4C0D1, #ED93B1); color: #4B1528; display: flex; align-items: center; justify-content: center; font-size: 8px; font-weight: 700; flex-shrink: 0; border: 1.5px solid #fff; box-shadow: 0 0 0 1px #E8D0CE; }
        ...
        .money-per { font-size: 9px; color: #639922; }
...
<div class="dh-body">
    ...
    <div class="branch-banner">
        ...
        <span>
            Showing all supervisors for <strong>Branch {{ $branchNo }}</strong>.
    ...
    <div class="stat-row">
        ...
        <div>
            <div class="stat-val">{{ $supervisors->count() }}</div>
            <div class="stat-lbl">Supervisors in Branch {{ $branchNo }}</div>
        </div>
    </div>
    ...
    <div class="search-row">
        <div class="search-wrap">
            ...
            <input class="search-input" type="text" placeholder="Search supervisor, staff ID…"
        ...
        <div class="tbl-wrap">
            <table>
                ...
                <tbody id="tableBody">
                    ...
                    <tr class="{{ $sup->staff_id === auth()->user()->staff_id ? 'is-me' : '' }}">
                        ...
                        <td>
                            <div class="av-wrap">
                                ...
                                <span>
                                    ...
                                    @if($sup->staff_id === auth()->user()->staff_id)
                                        <span class="badge badge-highlight" style="margin-left:6px;">You</span>
                                    @endif
                                </span>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="5">
                            <div class="empty-state">
                                ...
                                <p>No supervisors found for Branch {{ $branchNo }}.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <a href="{{ route('supervisor.dashboard') }}" class="back-link">
