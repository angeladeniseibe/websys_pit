...
<head>
    ...
    <title>Add Supervisor — DreamHome</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .trg {
            ...
            background: rgba(255,255,255,0.15); color: #F4C0D1;
            font-size: 9px; font-weight: 700;
            padding: 1px 7px; border-radius: 99px;
            letter-spacing: 0.04em; margin: 0 1px;
        }
        ...
        .form-sel:disabled { background: #F9F0EF; color: #bbb; cursor: not-allowed; }

        textarea.inp { resize: vertical; }

        .field-note { font-size: 10px; color: #B4B2A9; }
        .field-note.ok   { color: #27500A; font-weight: 600; }
        .field-note.warn { color: #A32D2D; font-weight: 600; }
        ...
        .bdg-supervisor { background: #EAF3DE; color: #27500A; }
...
<aside class="side-panel">
    <div class="side-logo">
        ...
        <div>
            ...
            <div class="side-logo-sub">Supervisor Network</div>
        </div>
    </div>

    <div class="side-heading">Create a supervisor record</div>
    <p class="side-sub">
        Register an existing Supervisor-position staff member by assigning
        them to a branch manager and documenting their responsibilities.
    </p>
    ...
    <ul class="field-guide">
        <li class="fg-item">
            ...
            <div>
                ...
                Only staff with position = Supervisor and no existing supervisor record are listed.
            </div>
        </li>
        <li class="fg-item">
            <div class="fg-icon"><i class="ti ti-building"></i></div>
            <div>
                <span class="fg-label">manager_no</span>
                The branch manager this supervisor reports to. Automatically filtered
                to the same branch as the selected supervisor.
            </div>
        </li>
        <li class="fg-item">
            <div class="fg-icon"><i class="ti ti-clipboard-text"></i></div>
            <div>
                <span class="fg-label">responsibility</span>
                A brief description of this supervisor's duties and scope at their branch.
            </div>
        </li>
    </ul>
    ...
    <div class="side-note">
        ...
        <span class="trg">Trigger 3</span> ensures supervisor and staff share the same branch.
    </div>

    <a href="{{ route('supervisors.index') }}" class="side-back">
        <i class="ti ti-arrow-left" style="font-size:13px;"></i> Back to supervisors
    </a>
</aside>

{{-- ── Right Form Area ── --}}
<main class="form-area">
    <div class="form-eyebrow">supervisor table — new record</div>
    <h1 class="form-title">Add new supervisor record</h1>
    ...
    <div class="preview-card" id="staff-preview">
        ...
        <div class="preview-grid">
            ...
            <div>
                ...
                <div class="preview-item-value"><span class="bdg bdg-supervisor">Supervisor</span></div>
            </div>
            <div>
        ...
        <div class="form-full field">
            ...
            <div class="select-wrap">
                <select class="form-sel {{ $errors->has('staff_id') ? 'has-err' : '' }}"
                    ...
                    <option value="">— Select a Supervisor from staff table —</option>
                    @foreach($availableSupervisors as $s)
            ...
            <span class="field-note">Only staff with position = Supervisor who do not yet have a supervisor record are shown.</span>
            @error('staff_id')<div class="field-err">{{ $message }}</div>@enderror
        </div>

        {{-- Manager Assignment --}}
        <div class="section-label">Manager Assignment</div>

        {{-- Hidden master list of all managers with branch data --}}
        <select id="all-managers-source" style="display:none;">
            @foreach($managers as $mgr)
                <option
                    value="{{ $mgr->staff_id }}"
                    data-branch="{{ $mgr->staff->branch_no ?? '' }}">
                    {{ $mgr->staff_id }} — {{ $mgr->staff->first_name ?? '' }} {{ $mgr->staff->last_name ?? '' }} ({{ $mgr->staff->branch_no ?? '' }})
                </option>
            @endforeach
        </select>

        <div class="form-full field">
            <label>manager_no <span class="req">*</span></label>
            <div class="select-wrap">
                <select name="manager_no" id="sel-manager" required disabled
                        class="form-sel {{ $errors->has('manager_no') ? 'has-err' : '' }}">
                    <option value="">— Select a supervisor first —</option>
                </select>
            </div>
            <span class="field-note" id="mgr-note">Select a supervisor above to load their branch manager.</span>
            @error('manager_no')<div class="field-err">{{ $message }}</div>@enderror
        </div>

        {{-- Responsibility --}}
        <div class="section-label">Role Details</div>
        <div class="form-full field">
            <label>responsibility <span class="req">*</span></label>
            <textarea name="responsibility" rows="3" required
                      class="inp {{ $errors->has('responsibility') ? 'has-err' : '' }}"
                      placeholder="e.g. Oversees property registrations and client intake for Branch B001">{{ old('responsibility') }}</textarea>
            <span class="field-note">Brief description of this supervisor's duties at their branch.</span>
            @error('responsibility')<div class="field-err">{{ $message }}</div>@enderror
        </div>

        <div class="form-actions">
            <button type="submit" class="btn-save">
                <i class="ti ti-device-floppy" style="font-size:15px;"></i> Save supervisor record
            </button>
...
<script>
    // ── snapshot all manager options from the hidden source on page load ──
    var allManagerOptions = [];

    document.addEventListener('DOMContentLoaded', function () {
        document.getElementById('all-managers-source')
            .querySelectorAll('option')
            .forEach(function (o) {
                allManagerOptions.push({
                    value  : o.value,
                    text   : o.text.trim(),
    ...
    function onStaffSelect() {
        ...
        var mgrNote         = document.getElementById('mgr-note');
        var oldManagerValue = "{{ old('manager_no') }}";

        // nothing selected — reset
        if (!sel.value) {
            preview.classList.remove('show');
            mgrSel.innerHTML    = '<option value="">— Select a supervisor first —</option>';
            mgrSel.disabled     = true;
            mgrNote.textContent = 'Select a supervisor above to load their branch manager.';
            mgrNote.className   = 'field-note';
            return;
        }
        ...
        // filter manager dropdown to same branch
        var supervisorBranch = opt.dataset.branch;
        var matched          = 0;

        mgrSel.innerHTML = '<option value="">— Select a Manager —</option>';

        allManagerOptions.forEach(function (m) {
            if (m.branch === supervisorBranch) {
                var newOpt            = document.createElement('option');
                newOpt.value          = m.value;
                newOpt.text           = m.text;
                newOpt.dataset.branch = m.branch;
                if (m.value === oldManagerValue) newOpt.selected = true;
                mgrSel.appendChild(newOpt);
                matched++;
            }
        });
        ...
        if (matched > 0) {
            ...
            mgrNote.textContent = 'Showing ' + matched + ' manager(s) from Branch ' + supervisorBranch + '.';
            mgrNote.className   = 'field-note ok';
        } else {
            mgrSel.disabled     = true;
            mgrNote.textContent = 'No manager found for Branch ' + supervisorBranch + '. Assign a manager to this branch first.';
            mgrNote.className   = 'field-note warn';
        }
    }
</script>
