    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .profile-av {
            ...
            background: linear-gradient(135deg, #72243E, #993556);
            color: #fff;
            display: flex; align-items: center; justify-content: center;
            font-size: 20px; font-weight: 700; flex-shrink: 0;
            border: 3px solid #fff;
            box-shadow: 0 2px 8px rgba(75,21,40,0.2);
        }
        ...
        .detail-row {
            ...
            padding: 7px 0; border-bottom: 0.5px solid #F5E8E6;
            gap: 12px;
        }
        ...
        .typing-card {
            background: #E6F1FB; border: 0.5px solid #B8D4F0;
            border-radius: 10px; padding: 14px 18px;
            display: flex; align-items: center; gap: 12px;
            margin-bottom: 14px;
        }
        .typing-icon { width: 38px; height: 38px; border-radius: 8px; background: #185FA5; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0; }
        .typing-val { font-size: 26px; font-weight: 500; color: #0C447C; line-height: 1; }
        .typing-lbl { font-size: 10px; color: #185FA5; text-transform: uppercase; letter-spacing: 0.07em; margin-top: 2px; }
...
<div class="dh-hero">
    ...
    <div class="dh-page-sub">
        ...
        Secretary · Personal staff details
    </div>
</div>
...
<div class="dh-body">
    ...
    <div class="readonly-note">
        ...
        <span>You have <strong>read-only</strong> access. Contact your manager to update your records.</span>
    </div>

    @if($secretary && $secretary->staff)
    ...
    <div class="profile-card">
        <div class="profile-card-head">
            ...
            <div>
                <div class="profile-name">{{ $secretary->staff->first_name }} {{ $secretary->staff->last_name }}</div>
                <div class="profile-pos">Secretary · Branch {{ $secretary->staff->branch_no }}</div>
                <span class="staff-id-badge">{{ $secretary->staff_id }}</span>
            </div>
        </div>
        ...
        <div class="detail-section">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">{{ $secretary->staff->sex === 'M' ? 'Male' : 'Female' }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Date of Birth</span>
                <span class="detail-value">{{ $secretary->staff->dob ?? '—' }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">NIN</span>
                <span class="detail-value mono">{{ $secretary->staff->nin ?? '—' }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Telephone</span>
                <span class="detail-value">{{ $secretary->staff->telephone ?? '—' }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Address</span>
                <span class="detail-value">{{ $secretary->staff->street ?? '—' }}, {{ $secretary->staff->city ?? '' }}</span>
            </div>
        </div>
        ...
        <div class="detail-section" style="border-top: 0.5px solid #EDD6D4;">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">{{ $secretary->staff->branch_no ?? '—' }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Position</span>
                <span class="detail-value">{{ $secretary->staff->position ?? '—' }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Date Joined</span>
                <span class="detail-value">{{ $secretary->staff->date_joined ?? '—' }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Salary</span>
                <span class="detail-value">£{{ number_format($secretary->staff->salary ?? 0, 0) }} / year</span>
            </div>
        </div>
    </div>
    ...
    <div class="typing-card">
        <div class="typing-icon"><i class="ti ti-keyboard"></i></div>
        <div>
            <div class="typing-val">{{ $secretary->typing_speed ?? '—' }} WPM</div>
            <div class="typing-lbl">Your Typing Speed</div>
        </div>
    </div>
    ...
    <a href="{{ route('secretary.dashboard') }}" class="back-link">
